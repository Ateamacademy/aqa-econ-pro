<!-- Auto-generated from /public/aqa-mocks/mark-scheme-paper-3-hard.pdf -->
AQA
...
7136/3 [Mark Scheme · Set B · Hard]
  ...
  Each question is worth 1 mark. The correct answers and brief justifications are shown in the grid below.
    ...
    Question     Answer                                        Brief explanation
       01          B       Diminishing marginal returns is a short-run concept where at least one factor is fixed
                           and variable factors are added.
       02          C       Revenue maximisation occurs where MR = 0 (TR is at its peak on a downward-sloping
                           demand curve).
       ...
       06          A       Appreciation makes imports cheaper in domestic currency, reducing the cost of
                           imported inputs and dampening cost-push inflation. Reject B (appreciation RAISES
                           foreign-currency price of exports), C (appreciation worsens CA), D (appreciation makes
                           country more expensive for foreign tourists).
       07          B       S − I = CA. If S falls with I unchanged, (S − I) falls, so CA worsens.
       08          C       Marshall-Lerner: if |εx| + |εm| > 1, depreciation improves trade balance after
                           adjustment. The short-run J-curve reflects delayed adjustment.
       09          A       Greater labour market flexibility reduces structural and frictional unemployment,
                           lowering the natural rate.
       ...
       14          A       Higher inflation (4.1%) than the anchor economy (Q: 1.8%) at a fixed rate means P's
                           ...
                           strain on the peg.
       15          B       Year 1: positive output gap (boom) but running a deficit → pro-cyclical. Year 3:
                           negative output gap (bust) and widened deficit → counter-cyclical.
       16          C       Allocative efficiency: MSB = MSC. At 60% abatement, MSB = MSC = £60.
       17          B       P = £10 at the profit-max point. P > MC (£10 > £6) → allocative inefficiency and
                           deadweight loss.
       ...
       21          B       Inelastic demand → consumers cannot easily substitute → most of tax passes through
                           as higher prices.
       22          B       Near LRAS, SRAS is steep: AD shifts produce large price-level rises and small output
                           rises.
       23          B       The MR discontinuity means MC can move within the gap without changing output →
                           prices remain rigid.
...
7136/3 [Mark Scheme · Set B · Hard]
    Question    Answer                                       Brief explanation
       ...
       26          B      Marshall-Lerner: long-run improvement depends on sum of export + import demand
                          elasticities > 1.
       27          B      Ad valorem is a percentage → absolute tax rises with price → high-priced goods bear
                          larger absolute tax.
       28          B      Further Lorenz curve from diagonal → larger Gini → higher inequality. Country Y is
                          further from the diagonal than X.
       29          C      At PED = 1 (unit elastic), small price changes leave total revenue essentially
                          unchanged.
       30          C      A Pigouvian tax equal to MEC shifts MPC up to coincide with MSC → market output
...
7136/3 [Mark Scheme · Set B · Hard]
  ...
  31         Using the data in the Extracts, examine the extent to which the UK has                 [10 marks]
             experienced a structural shift from goods to services in its trade performance
             since 2015.
...
7136/3 [Mark Scheme · Set B · Hard]
  33        Considering the information in Extract D and the original evidence in Extracts       [25 marks]
            ...
            long-run trade performance? Justify your recommendation.
