...
const PMT = "https://pmt.physicsandmathstutor.com/download/Economics/A-level/Past-Papers/Edexcel-A";
...
function fileLabel(y: number | "Specimen", type: "QP" | "MS", paper: number) {
  ...
  return `${prefix} ${type} - Paper ${paper} Edexcel (A) Economics A-level`;
}

export const edexcelAPastPapers: EdexcelPastPaper[] = [];

for (const paper of [1, 2, 3] as const) {
  for (const year of years) {
    edexcelAPastPapers.push({
      id: `edxa-p${paper}-${year}-qp`,
      paper,
      year,
      type: "Question Paper",
...
export const edexcelAPaperTitles: Record<number, string> = {
  1: "Markets & Business Behaviour",
  2: "The National & Global Economy",
