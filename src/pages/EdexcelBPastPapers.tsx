...
import { edexcelBPastPapers, edexcelBPaperTitles } from "@/data/edexcelBPastPapers";
import { getASPapers, asPaperTitles } from "@/data/asPastPapers";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";

export default function EdexcelBPastPapers() {
  const [search, setSearch] = useState("");
  const [yearFilter, setYearFilter] = useState<string>("all");
  const [level, setLevel] = useState<"alevel" | "as">("alevel");

  const asPapers = getASPapers("edexcel-b");

  const years = level === "alevel"
    ? [...new Set(edexcelBPastPapers.map((p) => p.year))].filter((y): y is number => typeof y === "number").sort((a, b) => b - a)
    : [...new Set(asPapers.map((p) => p.year))].filter((y): y is number => typeof y === "number").sort((a, b) => b - a);

  const filtered = (paper: number) =>
    edexcelBPastPapers.filter((p) => {
      ...
      if (search) {
        ...
        return p.type.toLowerCase().includes(q) || String(p.year).toLowerCase().includes(q) || edexcelBPaperTitles[p.paper].toLowerCase().includes(q);
      }
      return true;
    });
  ...
  return (
    <div className="container py-10">
      <div className="mb-8">
        <h1 className="text-3xl font-bold tracking-tight text-foreground mb-2">Edexcel A-Level Economics B Past Papers</h1>
        <p className="text-muted-foreground">Browse Edexcel Economics B (9EB0) past papers and mark schemes.</p>
      </div>
      ...
      {level === "alevel" ? (
        <Tabs defaultValue="1">
          ...
          {[1, 2, 3].map((paper) => (
            <TabsContent key={paper} value={String(paper)}>
              <p className="text-sm text-muted-foreground mb-4">{edexcelBPaperTitles[paper]}</p>
        ...
        <Tabs defaultValue="1">
          ...
          {[1, 2].map((paper) => (
            <TabsContent key={paper} value={String(paper)}>
              <p className="text-sm text-muted-foreground mb-4">{asPaperTitles["edexcel-b"][paper]}</p>
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
